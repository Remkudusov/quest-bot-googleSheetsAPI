...
bot = telebot.TeleBot("1872252534:AAEZlZ_B2y0KB75XGfooo-pcWtSVW5rG6Ks")
data = []

# The ID and range of a sample spreadsheet.
SAMPLE_SPREADSHEET_ID = '10PLz675Ugx5LNa7nsJ7tiXBTxFfCsQaA7P-B4L_WDHE'
# SAMPLE_RANGE_NAME = 'Лист1!A1:D1'
SAMPLE_RANGE_NAME_TO_READ = 'Лист1!A1:D1'
# Имя, Фамилия, Отчество, Почта

def insertInfo(name, surname, patronymic, email, service):
    with open('rows_counter.txt', 'r') as f:
        strings = int(f.read())

    SAMPLE_RANGE_NAME = 'Лист1!A' + str(strings) + ':D' + str(strings)
...
def start(message):
    ...
    data.clear()
    bot.register_next_step_handler(msg, detectName)

def detectName(message):
...
def detectSurname(message):
    ...
    bot.register_next_step_handler(msg, detectPatronymic)

def detectPatronymic(message):
    data.append(message.text)
    msg = bot.send_message(message.chat.id, "Введите параметр \"{}\" ".format(rows[0][3]))
    bot.register_next_step_handler(msg, detectEmail)

def detectEmail(message):
    data.append(message.text)
    insertInfo(data[0], data[1], data[2], data[3], service)
